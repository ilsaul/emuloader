...
uses
  ...
  AdvOfficeButtons, AdvGroupBox, CommCtrl, Buttons, uMain, EditEx,
  ButtonsEx;

type
  TGameInfoDeleteSingle = class(TEasyItemStored)
  private
...
type
  TFormDeleteGameFiles = class(TForm)
...
var
  FormDeleteGameFiles: TFormDeleteGameFiles;
...
function TGameInfoDeleteSingle.GetCaptions(Column: Integer): WideString;
var
  extraStr: String;
  FillBlank: String;
  ...
  case Column of
    ...
    1:
      begin
        ...
        case FormDeleteGameFiles.FilesListView.Scrollbars.VertBarVisible of
          True : VertBar:= 2;
          False: VertBar:= 0;
        end;
    ...
    2:
      begin
        if FormDeleteGameFiles.FilesListView.Scrollbars.VertBarVisible then
           FillBlank:= '104'
        else
           FillBlank:= '107';
...
function TGameInfoDeleteSingle.GetImageIndexes(Column: Integer): TCommonImageIndexInteger;
begin
  if Column = 0 then
     begin
...
procedure TFormDeleteGameFiles.Resize4K;
begin
  if not Is4KMode then
     Exit;
  with FormDeleteGameFiles do
  begin
    ClientWidth:=  1360;
    ClientHeight:= 1112;
    Font.Size:= 16;

    FormMain.Set4KEmuGameTopPanel(TopBar, SystemIcon, GameIcon, LabelGameTitle, 1205, LabelGameDetails, -1, LabelGameStatus, 1150);
    TopBar.Height:= 159;

    {FormMain.Set4KImageIconSpecs(SystemIcon, 128);
    FormMain.Set4KImageIconSpecs(GameIcon, 48, 143, 84);

    FormMain.Set4KLabelSpecs(LabelGameStatus, 1150, 81,  195, 51, 16);
    FormMain.Set4KLabelSpecs(LabelGameDetails, 199, 84,   -1, -1, 14);
    FormMain.Set4KLabelSpecs(LabelGameTitle,   144,  4, 1205, 59, 18);}

    FormMain.Set4KImageListSpecs(IL_MediaType, 68);

    //LabelEmulatorVersion.Left:= 199;
    //LabelEmulatorVersion.Top:= 107;
    //LabelEmulatorVersion.Font.Size:= 14;

    //LabelSoftwareList.Left:= 199;
    //LabelSoftwareList.Top:= 130;
    //LabelSoftwareList.Font.Size:= 14;

    //LabelSoftwareListTitle.Left:= 329;
    //LabelSoftwareListTitle.Top:= 130;
    //LabelSoftwareListTitle.Font.Size:= 14;

    FilesListView.CellSizes.Tile.Height:= IL_MediaType.Height+10; // 78;
    FilesListView.Height:= FilesListView.CellSizes.Tile.Height*10; // 780;
    FilesListView.Width:= ClientWidth-1;
    FilesListView.Top:= TopBar.Height+10;
    FilesListView.CellSizes.Tile.Width:= FilesListView.Width-1;
    FormMain.Set4KListViewCheckBoxHDSpecs(FilesListView); // HD checkbox
    //FilesListView.PaintInfoItem.CheckSize:= 22; // HD checkbox
    //FilesListView.CustomEnableIconHD:= True;

    BottomBar.Height:= 85;

    FormMain.Set4KButtonsOkCancelPanel(BottomBar, ButtonYes, ButtonNo, False);
    FormMain.Set4KButtonSpecs(ButtonHelp, ButtonYes.Left-10-68, 30, 68, 45, 16);
    ButtonYes.Top:=  30;
    ButtonNo.Top:=   30;

    FormMain.Set4KLabelSpecs(LabelTotalFiles,        699, 20, -1, -1, 16);
    FormMain.Set4KLabelSpecs(LabelTotalFilesChecked, 699, 53, -1, -1, 16);

    FormMain.Set4KCheckBoxSpecs(DeleteGameFromGamesList, 357, 15, 315, 36, 16);
    FormMain.Set4KCheckBoxSpecs(DeleteGameFileFromDisk,  357, 47, 315, 36, 16);

    FormMain.Set4KGroupBoxSpecs(FileTypesGroupBox, -1, -1, 335, 71);
...
procedure TFormDeleteGameFiles.ChangeCheckBoxColor(Enabled: Boolean; CheckBoxHolder: TAdvOfficeCheckBoxEx);
var
  Item: TEasyItem;
  sFileType: Integer;
begin
  ...
  repeat
    if FormMain.MemGameInfo.eIsCustomGame then
       ...
       begin
         if TGameInfoDeleteSingle(Item).eMediaType = sFileType then
            Item.Checked:= Enabled;
       end;
...
{procedure TFormDeleteGameFiles.LoadMediaIcons;
var
  Loop: ShortInt;
  Folder: String;
begin
...
procedure TFormDeleteGameFiles.AddFiles;
var
  addItem: TEasyItem;
...
begin
  ...
  if FileFullPath <> '' then
     begin
       // add .zip file
       addItem:= FilesListView.Items.AddCustom(TGameInfoDeleteSingle, nil);
       TGameInfoDeleteSingle(addItem).eSystemID:= FormMain.MemGameInfo.eSystemID;
       TGameInfoDeleteSingle(addItem).eCustomSystemID:= FormMain.MemGameInfo.eCustomSystemID;
       TGameInfoDeleteSingle(addItem).eROMIdentification:= FormMain.MemGameInfo.eROMIdentification;
       TGameInfoDeleteSingle(addItem).eMediaType:= 0;
       TGameInfoDeleteSingle(addItem).eCustomMediaType:= FormMain.MemGameInfo.eCustomMediaType;
       TGameInfoDeleteSingle(addItem).eFileType:= 0;
       TGameInfoDeleteSingle(addItem).eSoftwareName:= FormMain.MemGameInfo.eSoftwareName;
       TGameInfoDeleteSingle(addItem).eMerged:= GameIsMerged;
       TGameInfoDeleteSingle(addItem).eFileName:= FileFullPath;
       TGameInfoDeleteSingle(addItem).eFileSize:= GetFileSizeW(FileFullPath);
       TGameInfoDeleteSingle(addItem).eFileSizeText:= FormMain.GetSizeType(TGameInfoDeleteSingle(addItem).eFileSize, False);
       TGameInfoDeleteSingle(addItem).eDateTimeText:= FormMain.GetDateTimeStr(FileAgeW(FileFullPath));
       TGameInfoDeleteSingle(addItem).eParentFile:= GameIsMerged;
       TGameInfoDeleteSingle(addItem).eHeaderVerCHD:= 0;
       TGameInfoDeleteSingle(addItem).eIsCustomGame:= FormMain.MemGameInfo.eIsCustomGame;
       TGameInfoDeleteSingle(addItem).eIsUnicode:= FormMain.MemGameInfo.eIsUnicode;
                      ...
                      begin
                        ...
                        if DiskFile <> '' then
                           begin
                             ...
                             TGameInfoDeleteSingle(addItem).eSystemID:= FormMain.MemGameInfo.eSystemID;
                             TGameInfoDeleteSingle(addItem).eCustomSystemID:= FormMain.MemGameInfo.eCustomSystemID;
                             TGameInfoDeleteSingle(addItem).eMediaType:= 1;
                             TGameInfoDeleteSingle(addItem).eCustomMediaType:= FormMain.MemGameInfo.eCustomMediaType;
                             TGameInfoDeleteSingle(addItem).eFileType:= FileID;
                             TGameInfoDeleteSingle(addItem).eSoftwareName:= FormMain.MemGameInfo.eSoftwareName;
                             TGameInfoDeleteSingle(addItem).eMerged:= False;
                             TGameInfoDeleteSingle(addItem).eFileName:= DiskFile;
                             TGameInfoDeleteSingle(addItem).eFileSize:= GetFileSizeW(DiskFile);
                             TGameInfoDeleteSingle(addItem).eFileSizeText:= FormMain.GetSizeType(TGameInfoDeleteSingle(addItem).eFileSize, False);
                             TGameInfoDeleteSingle(addItem).eDateTimeText:= FormMain.GetDateTimeStr(FileAgeW(DiskFile));
                             TGameInfoDeleteSingle(addItem).eParentFile:= FormMain.GameIsClone(FormMain.MemGameInfo.eClone) and (not SameText(romName, ExtractFileNameW(DiskFile)));
                             TGameInfoDeleteSingle(addItem).eHeaderVerCHD:= HeaderVerCHD;
                             TGameInfoDeleteSingle(addItem).eIsCustomGame:= FormMain.MemGameInfo.eIsCustomGame;
                             TGameInfoDeleteSingle(addItem).eIsUnicode:= FormMain.MemGameInfo.eIsUnicode;
                             addItem.Details[1]:= 1;
                             addItem.Details[2]:= 2;
...
function TFormDeleteGameFiles.ProcessGamesFiles: Boolean;
begin
  ...
  FormCopyMoveGameFiles.Caption:= FormDeleteGameFiles.Caption;
...
procedure TFormDeleteGameFiles.AddConfigFiles;
var
  addItem: TEasyItem;
  FileFullPath: String;
  ...
  begin
    ...
    if Result then
       begin
          ...
          TGameInfoDeleteSingle(addItem).eSystemID:= FormMain.MemGameInfo.eSystemID;
          TGameInfoDeleteSingle(addItem).eCustomSystemID:= FormMain.MemGameInfo.eCustomSystemID;
          TGameInfoDeleteSingle(addItem).eMediaType:= -1; // for config files
          TGameInfoDeleteSingle(addItem).eFileType:= FileType;
          TGameInfoDeleteSingle(addItem).eSoftwareName:= FormMain.MemGameInfo.eSoftwareName;
          TGameInfoDeleteSingle(addItem).eMerged:= False;
          TGameInfoDeleteSingle(addItem).eFileName:= FileFullPath;
          TGameInfoDeleteSingle(addItem).eFileSize:= GetFileSizeW(FileFullPath);
          TGameInfoDeleteSingle(addItem).eFileSizeText:= FormMain.GetSizeType(TGameInfoDeleteSingle(addItem).eFileSize, False);
          TGameInfoDeleteSingle(addItem).eDateTimeText:= FormMain.GetDateTimeStr(FileAgeW(FileFullPath));
          TGameInfoDeleteSingle(addItem).eParentFile:= False;
          TGameInfoDeleteSingle(addItem).eHeaderVerCHD:= 0;
          TGameInfoDeleteSingle(addItem).eIsCustomGame:= FormMain.MemGameInfo.eIsCustomGame;
          TGameInfoDeleteSingle(addItem).eIsUnicode:= FormMain.MemGameInfo.eIsUnicode;
          addItem.Details[1]:= 1;
    ...
    if FormMain.IsMAMEBasedSys(FormMain.MemGameInfo.eSystemID) and (FileType = 1) and haveMAMEnvram then
       begin
         ...
         begin
           ...
           TGameInfoDeleteSingle(addItem).eROMIdentification:= FormMain.MemGameInfo.eSystemID;
           TGameInfoDeleteSingle(addItem).eSystemID:= FormMain.MemGameInfo.eSystemID;
           TGameInfoDeleteSingle(addItem).eCustomSystemID:= FormMain.MemGameInfo.eCustomSystemID;
           TGameInfoDeleteSingle(addItem).eMediaType:= -1; // for config files
           TGameInfoDeleteSingle(addItem).eCustomMediaType:= FormMain.MemGameInfo.eCustomMediaType;
           TGameInfoDeleteSingle(addItem).eFileType:= fixFileType;
           TGameInfoDeleteSingle(addItem).eSoftwareName:= FormMain.MemGameInfo.eSoftwareName;
           TGameInfoDeleteSingle(addItem).eMerged:= False;
           TGameInfoDeleteSingle(addItem).eFileName:= nvram_MAME[Loop];
           TGameInfoDeleteSingle(addItem).eFileSize:= GetFileSizeW(nvram_MAME[Loop]);
           TGameInfoDeleteSingle(addItem).eFileSizeText:= FormMain.GetSizeType(TGameInfoDeleteSingle(addItem).eFileSize, False);
           TGameInfoDeleteSingle(addItem).eDateTimeText:= FormMain.GetDateTimeStr(FileAgeW(nvram_MAME[Loop]));
           TGameInfoDeleteSingle(addItem).eParentFile:= False;
           TGameInfoDeleteSingle(addItem).eHeaderVerCHD:= 0;
           TGameInfoDeleteSingle(addItem).eIsCustomGame:= FormMain.MemGameInfo.eIsCustomGame;
           TGameInfoDeleteSingle(addItem).eIsUnicode:= FormMain.MemGameInfo.eIsUnicode;
...
procedure TFormDeleteGameFiles.UpdateTotalCheckedLabel;
begin
  LabelTotalFilesChecked.Caption:= Format('%.2u Files Checked', [FilesListView.CheckManager.Count]);
end;

procedure TFormDeleteGameFiles.FormShow(Sender: TObject);
var
  HeightDiff, Loop: Integer;
  Screen480, ScreenHighRes, Screen4K: Boolean;
begin
  Resize4K;
  ...
  begin
    ...
    SetFormColors(FormDeleteGameFiles, TopBar, BottomBar, LabelGameTitle, LabelGameDetails, LabelGameStatus, FormMain.MemGameInfo.eGameSetStatus, IsNightMode);

    for Loop:= 0 to FormDeleteGameFiles.ComponentCount-1 do
    begin
      if FormDeleteGameFiles.Components[Loop] is TBitBtnEx then
         FormMain.SetButtonExColors(TBitBtnEx(FormDeleteGameFiles.Components[Loop]))
      else
      if FormDeleteGameFiles.Components[Loop] is TAdvOfficeCheckBoxEx then
      begin
        SetCheckBoxColors(TAdvOfficeCheckBoxEx(FormDeleteGameFiles.Components[Loop]), clCream, item_caption_active_shadow_color[1], clrMedDarkGray, clrLightBlack);
        FormMain.SetCheckBoxExCustomIcon(TAdvOfficeCheckBoxEx(FormDeleteGameFiles.Components[Loop]));
      end;
    end;

    SetLabelColors(LabelGameDetails,       LabelGameDetails.Font.Color, LabelGameDetails.ShadowColor);

    FormMain.SetEasyListViewColors(FilesListView, FormDeleteGameFiles.Color, clWhite);
    FormMain.ELV_SetCheckRadioCustomIcon(FilesListView);

    PanelDestinationFolder.Color1:= FormDeleteGameFiles.Color;
  ...
  FormMain.LoadGameIcon(GameIcon, FormMain.MemGameInfo.eIsCustomGame);
  ...
  Screen4K:= Is4KMode and (Screen.Height = 2160);
  ScreenHighRes:= (Screen.Height > 800) and (not Screen4K);
  ...
  if Screen4K then
     begin
       LabelTotalFiles.Left:= 530;
       LabelTotalFilesChecked.Left:= 530;
     end
  else
     begin
  ...
  if Screen.Width < 800 then
     begin
       HeightDiff:= FormDeleteGameFiles.Width-(Screen.Width-15); //635; 715
       //HeightDiff:= FormDeleteGameFiles.Width-(640-15); // for debugging only
       ...
       FormDeleteGameFiles.Width:= FormDeleteGameFiles.Width-HeightDiff;
     end
  else
  if ScreenHighRes then
     begin
       ...
       FormDeleteGameFiles.Height:= FormDeleteGameFiles.Height+HeightDiff;
     end;
    ...
    False:
      begin
        ...
        if FormMain.EmulatorVersion[FormMain.MemGameInfo.eSystemID] <> '' then
           LabelGameDetails.Caption:= LabelGameDetails.Caption+#13#10+FormMain.EmulatorVersion[FormMain.MemGameInfo.eSystemID];
        //else
        //   begin
        //     // why do vert centering of the game name label ?
        ...
        if FormMain.MemGameInfo.eSoftwareName <> '' then
           LabelGameDetails.Caption:= LabelGameDetails.Caption+#13#10+'software list: '+FormMain.MemGameInfo.eCategory;
        LabelGameDetails.Canvas.UnLock;
  ...
  if ActionMode = 0 then
     begin
       ...
       FormDeleteGameFiles.Height:= FormDeleteGameFiles.Height-PanelDestinationFolder.Height;
     end
  else
     begin
  ...
  if FilesListView.Scrollbars.VertBarVisible then
     ...
     begin
       ...
       if ((Screen4K) and (FilesListView.Groups.ItemCount < 10)) or
          ((not Screen480) and (not ScreenHighRes) and (FilesListView.Groups.ItemCount < 6)) or
       ...
       //if FilesListView.Groups.Count < 6 then
          begin
            ...
            HeightDiff:= HeightDiff-FilesListView.Height;
            FormDeleteGameFiles.ClientHeight:= FormDeleteGameFiles.ClientHeight-HeightDiff;
          end;
       //else
  ...
  case FormMain.CheckTotal(FilesListView) of
    ...
    False:
      begin
        ...
        case FormMain.MemGameInfo.eIsCustomGame of
          True:
            begin
              ...
              FormMain.AddGamesListHeaderConsComp;
          ...
          False:
            begin
              ...
              FormMain.AddEmulatorHeaderArcade(True);
              if FormMain.MemGameInfo.eSoftwareName <> '' then
        ...
        FormMain.ShowMessageBox(FormDeleteGameFiles.Caption, FormMain.MemGameInfo.eTitle, '', 2, False, -1);
...
procedure TFormDeleteGameFiles.FilesListViewItemPaintText(
...
procedure TFormDeleteGameFiles.FilesListViewKeyAction(
...
procedure TFormDeleteGameFiles.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  case Key of
    //#13: ButtonYes.Click;
...
procedure TFormDeleteGameFiles.GetFilesCountSize;
var
  Item: TEasyItem;
begin
  ...
  repeat
    if Item.Checked then
       begin
         case TGameInfoDeleteSingle(Item).eMediaType of
           0: // ROMs
             begin
               ROMsTotalSize:= ROMsTotalSize+TGameInfoDeleteSingle(Item).eFileSize;
               Inc(ROMsTotalFiles);
             end;
           1: // CHDs
             begin
               CHDsTotalSize:= CHDsTotalSize+TGameInfoDeleteSingle(Item).eFileSize;
               Inc(CHDsTotalFiles);
             end;
           -1: // CFGs
             begin
               CFGsTotalSize:= CFGsTotalSize+TGameInfoDeleteSingle(Item).eFileSize;
               Inc(CFGsTotalFiles);
             end;
         end;
       end;
...
procedure TFormDeleteGameFiles.ButtonYesClick(Sender: TObject);
var
  FileTypeStr: String;
...
begin
  ...
  if not FormMain.HaveItemsChecked(FilesListView) then
     begin
       FormMain.InitMessageBox;
       FormMain.ShowGameNameEntryMsgBox;
       FormMain.ShowMessageBox(FormDeleteGameFiles.Caption, FormMain.MemGameInfo.eTitle, '    You haven''t checked '+
  ...
  if ActionMode <> 0 then
     begin
       ...
       begin
         FormMain.InitMessageBox;
         FormMain.ShowGameNameEntryMsgBox;
         FormMain.ShowMessageBox(FormDeleteGameFiles.Caption, FormMain.MemGameInfo.eTitle, '    You haven''t selected a destination '+
     ...
     begin
        ...
        if FileTypeStr = '' then
           begin
             FormMain.InitMessageBox;
             FormMain.ShowGameNameEntryMsgBox;
             FormMain.ShowMessageBox(FormDeleteGameFiles.Caption, FormMain.MemGameInfo.eTitle, '    Could not detect the destination drive letter. '+
  ...
  if FormMain.ShowMessageBox(FormDeleteGameFiles.Caption, FormMain.MemGameInfo.eTitle, '', 1, True, -1) = mrNo then
     Exit;
...
procedure TFormDeleteGameFiles.FormCloseQuery(Sender: TObject;
...
procedure TFormDeleteGameFiles.FilesListViewItemCheckChange(
...
procedure TFormDeleteGameFiles.DeleteCHDsClick(Sender: TObject);
begin
  ChangeCheckBoxColor(TAdvOfficeCheckBoxEx(Sender).Checked, TAdvOfficeCheckBoxEx(Sender));
  if FormDeleteGameFiles.Visible then
     FilesListView.SetFocus;
end;

procedure TFormDeleteGameFiles.ButtonSelectROMsFolderClick(
  Sender: TObject);
begin
  FormMain.DialogSelectFolder(DestinationFolder, False);
end;

procedure TFormDeleteGameFiles.ButtonHelpClick(Sender: TObject);
begin
  FormMain.InitMessageBox;
  FormMain.AddMsgText('    You can choose what files will be processed in the ');
  FormMain.AddMsgText('Check Arcade File Types To '+ActionString, MsgTxtColors.colorKeyTitle, [fsBold]);
  FormMain.AddMsgText(' group box. These settings are for ');
  FormMain.AddMsgText('MAME and arcade', MsgTxtColors.colorKeyTitle, [fsBold]);
  ...
  FormMain.AddMsgText('console/computer/handheld', MsgTxtColors.colorExitCode, [fsBold]);
  FormMain.AddMsgText(' systems, check ');
  FormMain.AddMsgText('Delete Game File From Disk', MsgTxtColors.colorKeyTitle, [fsBold]);
  ...
  FormMain.AddMsgText('Delete Game From Games List', MsgTxtColors.colorKeyTitle, [fsBold]);
  FormMain.AddMsgText('. This option is not compatible with MAME and arcade games.'+#13#10+#13#10+
                      '    To ');
  FormMain.AddMsgText(LowerCase(ActionString), MsgTxtColors.colorMachineName, [fsBold]);
  FormMain.AddMsgText(' files, click ');
  FormMain.AddMsgText(ActionString+' Files', MsgTxtColors.colorMachineName, [fsBold]);
