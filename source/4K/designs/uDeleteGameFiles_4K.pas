...
type
  TFormDeleteGameFiles4K = class(TForm)
...
var
  FormDeleteGameFiles4K: TFormDeleteGameFiles4K;
  ...
  case Column of
    ...
    1:
      begin
        ...
        case FormDeleteGameFiles4K.FilesListView.Scrollbars.VertBarVisible of
          True : VertBar:= 2;
          False: VertBar:= 0;
        end;
    ...
    2:
      begin
        if FormDeleteGameFiles4K.FilesListView.Scrollbars.VertBarVisible then
           FillBlank:= '104'
        else
           FillBlank:= '107';
...
procedure TFormDeleteGameFiles4K.ChangeCheckBoxColor(Enabled: Boolean; CheckBoxHolder: TAdvOfficeCheckBoxEx);
var
  Item: TEasyItem;
  sFileType: Integer;
begin
...
procedure TFormDeleteGameFiles4K.LoadMediaIcons;
var
  Loop: ShortInt;
  Folder: String;
begin
...
procedure TFormDeleteGameFiles4K.AddFiles;
var
  addItem: TEasyItem;
...
function TFormDeleteGameFiles4K.ProcessGamesFiles: Boolean;
begin
  ...
  FormCopyMoveGameFiles.Caption:= FormDeleteGameFiles4K.Caption;
...
procedure TFormDeleteGameFiles4K.AddConfigFiles;
var
  addItem: TEasyItem;
  FileFullPath: String;
...
procedure TFormDeleteGameFiles4K.UpdateTotalCheckedLabel;
begin
  LabelTotalFilesChecked.Caption:= Format('%.2u Files Checked', [FilesListView.CheckManager.Count]);
end;

procedure TFormDeleteGameFiles4K.FormShow(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    SetFormColors(FormDeleteGameFiles4K, TopBar, BottomBar, LabelGameTitle, LabelGameDetails, LabelGameStatus, FormMain.MemGameInfo.eGameSetStatus, IsNightMode);

    for Loop:= 0 to FormDeleteGameFiles4K.ComponentCount-1 do
    begin
      if FormDeleteGameFiles4K.Components[Loop] is TBitBtnEx then
         FormMain.SetButtonExColors(TBitBtnEx(FormDeleteGameFiles4K.Components[Loop]))
      else
      if FormDeleteGameFiles4K.Components[Loop] is TAdvOfficeCheckBoxEx then
      begin
        SetCheckBoxColors(TAdvOfficeCheckBoxEx(FormDeleteGameFiles4K.Components[Loop]), clCream, item_caption_active_shadow_color[1], clrMedDarkGray, clrLightBlack);
        FormMain.SetCheckBoxExCustomIcon(TAdvOfficeCheckBoxEx(FormDeleteGameFiles4K.Components[Loop]));
      end;
    end;
    ...
    FormMain.SetEasyListViewColors(FilesListView, FormDeleteGameFiles4K.Color, clWhite);
    FormMain.ELV_SetCheckRadioCustomIcon(FilesListView);

    PanelDestinationFolder.Color1:= FormDeleteGameFiles4K.Color;
  ...
  if Screen.Width < 800 then
     begin
       HeightDiff:= FormDeleteGameFiles4K.Width-(Screen.Width-15); //635; 715
       //HeightDiff:= FormDeleteGameFiles4K.Width-(640-15); // for debugging only
       ...
       FormDeleteGameFiles4K.Width:= FormDeleteGameFiles4K.Width-HeightDiff;
     end
  else
  if ScreenHighRes then
     begin
       ...
       FormDeleteGameFiles4K.Height:= FormDeleteGameFiles4K.Height+HeightDiff;
     end;
  ...
  if ActionMode = 0 then
     begin
       ...
       FormDeleteGameFiles4K.Height:= FormDeleteGameFiles4K.Height-PanelDestinationFolder.Height; // 50
     end
  else
     begin
  ...
  case FormMain.CheckTotal(FilesListView) of
    ...
    False:
      begin
        ...
        GenerateMessage(FormDeleteGameFiles4K.Caption, FormMain.MemGameInfo.eTitle, '', 2, False, -1);
...
procedure TFormDeleteGameFiles4K.FilesListViewItemPaintText(
...
procedure TFormDeleteGameFiles4K.FilesListViewKeyAction(
...
procedure TFormDeleteGameFiles4K.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  case Key of
    //#13: ButtonYes.Click;
...
procedure TFormDeleteGameFiles4K.GetFilesCountSize;
var
  Item: TEasyItem;
begin
...
procedure TFormDeleteGameFiles4K.ButtonYesClick(Sender: TObject);
var
  FileTypeStr: String;
...
begin
  ...
  if not FormMain.HaveItemsChecked(FilesListView) then
     begin
       ...
       FormMain.ShowGameNameEntryMsgBox;
       GenerateMessage(FormDeleteGameFiles4K.Caption, FormMain.MemGameInfo.eTitle, '    You haven''t checked '+
  ...
  if ActionMode <> 0 then
     begin
       ...
       begin
         FormMain.InitMessageBox; //CallMessageBox;
         FormMain.ShowGameNameEntryMsgBox;
         GenerateMessage(FormDeleteGameFiles4K.Caption, FormMain.MemGameInfo.eTitle, '    You haven''t selected a destination '+
     ...
     begin
        ...
        if FileTypeStr = '' then
           begin
             FormMain.InitMessageBox; //CallMessageBox;
             FormMain.ShowGameNameEntryMsgBox;
             GenerateMessage(FormDeleteGameFiles4K.Caption, FormMain.MemGameInfo.eTitle, '    Could not detect the destination drive letter. '+
  ...
  if GenerateMessage(FormDeleteGameFiles4K.Caption, FormMain.MemGameInfo.eTitle, '', 1, True, -1) = mrNo then
     Exit;
...
procedure TFormDeleteGameFiles4K.FormCloseQuery(Sender: TObject;
...
procedure TFormDeleteGameFiles4K.FilesListViewItemCheckChange(
...
procedure TFormDeleteGameFiles4K.DeleteCHDsClick(Sender: TObject);
begin
  ChangeCheckBoxColor(TAdvOfficeCheckBoxEx(Sender).Checked, TAdvOfficeCheckBoxEx(Sender));
  if FormDeleteGameFiles4K.Visible then
     FilesListView.SetFocus;
end;

procedure TFormDeleteGameFiles4K.ButtonSelectROMsFolderClick(
  Sender: TObject);
begin
  FormMain.DialogSelectFolder(DestinationFolder, False);
end;

procedure TFormDeleteGameFiles4K.ButtonHelpClick(Sender: TObject);
begin
  FormMain.InitMessageBox; //CallMessageBox;
