...
type
  ...
  private
    ...
    LayoutInfo: packed array[0..8] of TLayoutInfo; // [0..23]
    ...
    procedure LoadLayoutImage(Index: Byte);
  ...
  repeat
    ...
    if Item.ImageIndex > 2 then
       begin
         ...
         //if Item.ImageIndex < 9 then // tripple max index = 8; index 9 to 15 is quad layouts (cannot hide panels!!!)
    ...
    //   end;
    if Item.ImageIndex > 0 then //  if Item.ImageIndex in [1..8] then
...
begin
  ...
  repeat
    ...
    //if Item.ImageIndex > 8 then // for quad layouts
    ...
    if Item.ImageIndex > 2 then // if Item.ImageIndex in [3..8] then
...
procedure TFormCategoryLayoutSettings.LoadLayoutImage(Index: Byte);
var
  layFile: String;
begin
...
begin
  ...
  begin
    ...
    //LayoutInfo[Loop].lImage4_imgCategory:= GetScrLayoutDefaultType(Loop, 4); // for quad layouts
    LayoutInfo[Loop].lImage2_Enabled:= Loop > 0; // Loop in [1..8]
    LayoutInfo[Loop].lImage3_Enabled:= Loop > 2; // Loop in [3..8]
...
begin
  ...
  PanelEnabledScr2.Visible:= LayoutSelectedItem.ImageIndex > 0; // .ImageIndex in [1..8]
  PanelEnabledScr3.Visible:= LayoutSelectedItem.ImageIndex > 2; // .ImageIndex in [3..8]
  ...
  {case PanelEnabledScr4.Visible of
    True:
      begin
        PanelEnabledScr4.Checked:= LayoutInfo[LayoutSelectedItem.ImageIndex].lImage4_Enabled;
        LoadLayoutIcon(IconLayScr4, LayoutInfo[LayoutSelectedItem.ImageIndex].lImage4_imgCategory); // new
        LabelLayScr4.Caption:= FormMain.PopupMenuImageCategories.Items[LayoutInfo[LayoutSelectedItem.ImageIndex].lImage4_imgCategory].Caption; // new
        if PanelEnabledScr4.Checked then
           begin
             if PanelEnabledScr4.Checked and (not IconLayScr4.Visible) then
                begin
                  IconLayScr4.Visible:= True;
                  LabelLayScr4.Visible:= True;
                end;
           end
        else
           begin
             IconLayScr4.Visible:= False;
             LabelLayScr4.Visible:= False;
           end;
      end;
    False:
      begin
...
procedure TFormCategoryLayoutSettings.ButtonCategoryToggleVisibilityClick(
  Sender: TObject);
var
  Item: TEasyItem;
begin
  ...
  case TBitBtn(Sender).Tag of
    0: TBitBtn(Sender).Caption:= 'Hide All';
    1: TBitBtn(Sender).Caption:= 'Show All';
  end;
  ELV_SetFocus;
end;
...
begin
  ...
  TBitBtn(Sender).Tag:= Ord(not Boolean(TBitBtn(Sender).Tag));
  case TBitBtn(Sender).Tag of
    0: TBitBtn(Sender).Caption:= 'Hide All Layouts';
    1: TBitBtn(Sender).Caption:= 'Show All Layouts';
  end;
  ELV_SetFocus;
end;
...
begin
  ...
  FormMain.ELV_PopulateSystems(ImageCategorySystem_Selector, True, 1);
  ...
  FormCategoryLayoutSettings.Left:= (Screen.Width shr 1)-(Width shr 1)-1;
end;
...
begin
  // categories -> ClientWidth:= 640; ClientHeight:= 430;
  // layouts -> ClientWidth:= 640; ClientHeight:= 430;
