...
type
  TFormScanAudioSamples = class(TForm)
...
var
  FormScanAudioSamples: TFormScanAudioSamples;
...
procedure TFormScanAudioSamples.ClearSelected;
begin
  FilesListView.Selection.ClearAll;
  SelectedItem:= nil;
end;

procedure TFormScanAudioSamples.SetSelectedGame(AutoSelect: Boolean = True);
begin
...
procedure TFormScanAudioSamples.DeleteEmptyGroups;
var
  Group: TEasyGroup;
begin
...
procedure TFormScanAudioSamples.SetFilter;
var
  Item: TEasyItem;
begin
...
procedure TFormScanAudioSamples.SelectFirstVisibleItem;
begin
...
procedure TFormScanAudioSamples.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #27 then
     Close;
...
procedure TFormScanAudioSamples.FilesListViewItemPaintText(
...
procedure TFormScanAudioSamples.FilesListViewItemSelectionChanged(
  Sender: TCustomEasyListview; Item: TEasyItem);
begin
...
procedure TFormScanAudioSamples.PopupPlayGameClick(Sender: TObject);
var
  GameEasy: TEasyItem;
  RunGame: Boolean;
  ...
  begin
    GenerateMessage('Error', FormMain.GetEmulatorDescription(TFileInfo(SelectedItem).eROMIdentification),
...
procedure TFormScanAudioSamples.PopupRemoveSelectedClick(Sender: TObject);
begin
...
procedure TFormScanAudioSamples.PopupClearListClick(Sender: TObject);
begin
  ClearSelected;
  FormMain.ClearListView(FilesListView);
  FilesListView.SetFocus;
...
procedure TFormScanAudioSamples.PopupRebuildListClick(Sender: TObject);
begin
  PopupClearList.Click;
  AddGamesToList;
end;

procedure TFormScanAudioSamples.PopupShowAllGamesClick(Sender: TObject);
begin
  SetFilter;
end;

procedure TFormScanAudioSamples.MenuSaveListToFileClick(Sender: TObject);
var
  Group: TEasyGroup;
  Item: TEasyItem;
...
begin
  ...
  case FormScanAudioSamples.Tag of
  ...
  GenerateMessage(FormScanAudioSamples.Caption, 'Save audio samples list to a text file.',
                  Format('    File "%s" was created based on the current list.',
                         [FileStr]), 2);
end;

procedure TFormScanAudioSamples.FilesListViewKeyAction(
...
procedure TFormScanAudioSamples.FilesListViewDblClick(
...
procedure TFormScanAudioSamples.PopupAudioSamplesMeasureMenuItem(
...
procedure TFormScanAudioSamples.FilesListViewColumnClick(
  Sender: TCustomEasyListview; Button: TCommonMouseButton;
...
function TFormScanAudioSamples.FilesListViewItemCompare(
...
procedure TFormScanAudioSamples.LabelDownloadLinkMouseEnter(Sender: TObject);
begin
  LabelDownloadLink.Font.Color:= clBlue;
end;

procedure TFormScanAudioSamples.LabelDownloadLinkMouseLeave(
  Sender: TObject);
begin
  LabelDownloadLink.Font.Color:= clNavy;
end;

procedure TFormScanAudioSamples.LabelDownloadLinkClick(Sender: TObject);
begin
  ShellExecute(Handle, 'open', PAnsiChar(LabelDownloadLink.Hint), nil, nil, SW_SHOWNORMAL);
end;
