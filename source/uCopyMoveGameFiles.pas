...
begin
  ...
  if Assigned(FormZipFilesNotUsed) then
     begin
       MultiSystems:= FormZipFilesNotUsed.ZipFilesListView.Groups.Count > 1;
       LabelGameNameCloneOf.Visible:= False;
     end;
  ...
  if Assigned(FormZipFilesNotUsed) then
     DestinationFullPath:= FormZipFilesNotUsed.DestinationFolder.Text;
...
var
  ...
  tmpFileName, FileNoPath, CopyMoveToDir: String;
...
begin
  ...
  if Assigned(FormZipFilesNotUsed) then
     begin
       tmpMediaType:= 0; // ROMs
       fixFileType:= 0; // always .cfg type as this feature only handle ROMs (.zip; .7z formats)
       FormZipFilesNotUsed.ZipFilesListView.BeginUpdate;
       Group:= FormZipFilesNotUsed.ZipFilesListView.Groups.FirstGroup;
       repeat
         // 0: delete all files
         // 1: move selected files only
         // 2: move all files
         // 3: delete selected files only
         SetSysIcon(uZipFilesNotUsed.TSysGroupInfo(Group).eSystemID);
         LabelGameTitle.Caption:= uZipFilesNotUsed.TSysGroupInfo(Group).eTitle;
         tmpFileName:= '';

         Item:= FormZipFilesNotUsed.ZipFilesListView.Groups.LastInGroup(Group);
         repeat
           ItemToDelete:= nil;
           if FormZipFilesNotUsed.Tag in [0, 2] then
              Continue:= True
           else
              Continue:= Item.Selected;
           tmpFileName:= uZipFilesNotUsed.TZipGameInfo(Item).eFullPath+uZipFilesNotUsed.TZipGameInfo(Item).eFileName;
           if Continue then
              Continue:= FileExists(tmpFileName);
           if Continue then
              begin
                CopyMoveToDir:= DestinationFullPath;
                if MultiSystems then
                   begin
                     if uZipFilesNotUsed.TZipGameInfo(Item).eSystemID <> idMAME then
                        CopyMoveToDir:= CopyMoveToDir+FormMain.GetSystemIniSection(uZipFilesNotUsed.TZipGameInfo(Item).eSystemID)+'\';
                   end;
                ...
                FileNoPath:= ExtractFileName(uZipFilesNotUsed.TZipGameInfo(Item).eFileName);

                tmpFileSize:= uZipFilesNotUsed.TZipGameInfo(Item).eSize;//GetFileSize(tmpFileName);
                FileSizeText:= uZipFilesNotUsed.TZipGameInfo(Item).eSizeText;//FormMain.GetSizeType(tmpFileSize, False);
                DateTimeText:= uZipFilesNotUsed.TZipGameInfo(Item).eDateTimeText;//FormMain.GetDateTimeStr(FileAge(tmpFileName));
                  ...
                  0: // delete file
                    begin
                      case DeleteFile(tmpFileName) of
                        True:
                          begin
                            ItemToDelete:= Item; // cannot delete selected TEasyItem before selecting another one!!!
                          end;
                        False:
                          begin
                            CheckLogHeader;
                            ...
                            Inc(OperationErrors);
                            iFilesSizeFailed:= iFilesSizeFailed+tmpFileSize;
                          end;
                      end;
                    end;
                  2: // move file
                    begin
                      ...
                      case MoveFile(tmpFileName, CopyMoveToDir+FileNoPath, OverwriteFiles) of
                        True:
                          begin
                            ItemToDelete:= Item;
                          end;
              ...
              end; // end of FileExists() and Continue
           Inc(CurrentFile);
           ...
           Item:= FormZipFilesNotUsed.ZipFilesListView.Groups.PrevInGroup(Group, Item);
           if ItemToDelete <> nil then
              FormZipFilesNotUsed.ZipFilesListView.Groups.DeleteItem(ItemToDelete);
         until (Item = nil) or (ButtonCancel.ModalResult = mrCancel);
         Application.ProcessMessages;
         Group:= FormZipFilesNotUsed.ZipFilesListView.Groups.NextGroup(Group);
       until (Group = nil) or (ButtonCancel.ModalResult = mrCancel);
       CheckCancelButton;
       Application.ProcessMessages;

       FormZipFilesNotUsed.ZipFilesListView.EndUpdate;
       FormMain.ELV_RemoveEmptyGroups(FormZipFilesNotUsed.ZipFilesListView); // delete empty groups
       FormZipFilesNotUsed.LabelTotalItems.Caption:= Format(FormZipFilesNotUsed.LabelTotalItems.Hint,
                       [FormZipFilesNotUsed.ZipFilesListView.Groups.ItemCount, FormMain.GetSizeType(iFilesSizeFailed, False)]);
     end
  else
            ...
            if Assigned(uDeleteMultipleGamesFiles.TGameInfo(Item).eGameFiles) and (uDeleteMultipleGamesFiles.TGameInfo(Item).eGameFiles.Count > 0) then
               begin
                 ...
                 if uDeleteMultipleGamesFiles.TGameInfo(Item).eMerged then
                    tmpFileName:= tmpFileName+' [merged]';
