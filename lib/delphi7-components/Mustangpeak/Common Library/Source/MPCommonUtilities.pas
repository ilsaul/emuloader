...
function FileExistsW(const FileName: WideString): Boolean; overload;
function FileExistsW(const FileName: WideString; out DOSName: String): Boolean; overload;
...
// added by Ciro Alfredo Consentino - August 06, 2014
function FileExistsW(const FileName: WideString; out DOSName: String): Boolean;
{$IFNDEF TNT}
var
  Handle: THandle;
  FindDataA: TWin32FindDataA;
...
begin
  ...
  if IsUnicode then
    begin
      Handle := FindFirstFileW_MP(PWideChar(FileName), FindDataW);
      DOSName := FindDataW.cAlternateFileName;
    end
  else
    begin
      Handle := FindFirstFileA(PAnsiChar(AnsiString(FileName)), FindDataA);
      DOSName := FindDataA.cAlternateFileName;
    end;
  Result:= Handle <> INVALID_HANDLE_VALUE;
  if Result then
     Windows.FindClose(Handle)
