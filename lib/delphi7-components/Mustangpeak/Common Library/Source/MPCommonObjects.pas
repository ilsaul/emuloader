...
type
  ...
  private
    ...
    FShowThemedBorderColor: TColor; // added by Ciro Alfredo Consentino (July 07, 2018)
    ...
    procedure SetShowThemedBorderColor(const Value: TColor); // added by Ciro Alfredo Consentino (July 07, 2018)
  ...
  protected
    ...
    property ShowThemedBorderColor: TColor read FShowThemedBorderColor write SetShowThemedBorderColor default clNone; // clNone will use Windows themed border color
...
begin
  ...
  FShowThemedBorderColor := clNone; // added by Ciro Aflredo Consentino (July 07, 2018)
...
procedure TCommonCanvasControl.SetShowThemedBorderColor(const Value: TColor); // added by Ciro Alfredo Consentino (July 07, 2018)
begin
  if Value <> FShowThemedBorderColor then
  begin
    FShowThemedBorderColor := Value;
    if HandleAllocated then
       RecreateWnd; // Need to modify the NonClient area
  end;
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        // added this to fix white square when both hor/vert scroll bars are visible (Win10 dark mode mode)
        NCCanvas.Brush.Color:= GetPixel(NCCanvas.Handle, ClientR.Right-2, Filler.Top-2); // NCCanvas.Brush.Color := clBtnFace; // original code
