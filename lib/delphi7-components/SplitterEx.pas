...
uses
  ...
  // 1.3.0.1 : Added single color style (by Ciro Alfredo Consentino, March 25, 2012)
...
type
  ...
  // TTMSStyle
  TTMSStyle = (tsSilver, tsSolidColor, tsOffice2003Blue, tsOffice2003Silver, tsOffice2003Olive, tsOffice2003Classic,
    tsOffice2007Luna, tsOffice2007Silver, tsOffice2007Obsidian, tsWindowsXP, tsWhidbey, tsWindowsVista,
    tsWindows7, tsTerminal, tsOffice2010Blue, tsOffice2010Silver, tsOffice2010Black, tsCustom);
  ...
  private
    ...
    procedure SetSingleColorHot(const Value: TColor);
  protected
  ...
  published
    ...
    property SingleColorHot: TColor read FSingleColorHot write SetSingleColorHot;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  end;

  TCustomSplitterEx = class(TSplitter)
  private
    ...
    FStyle: TTMSStyle; // added by Ciro Alfredo Consentino, March 10, 2011
  ...
  protected
    ...
    property  Style: TTMSStyle read FSTYLE write SetComponentStyle default tsSilver; // added by Ciro Alfredo Consentino, March 10, 2011
  public
  ...
  TSplitterEx = class(TCustomSplitterEx)
  published
    property Appearance;
...
procedure Register;
begin
  RegisterComponents('Components', [TSplitterEx]);
end;
...
begin
  ...
  FSingleColor:= clBtnFace;
  FSingleColorHot:= clGray;
end;
...
begin
  ...
  begin
    ...
    FSingleColor:= (Source as TSplitterAppearance).SingleColor;
    FSingleColorHot:= (Source as TSplitterAppearance).SingleColorHot;
  end;
end;
...
procedure TSplitterAppearance.SetSingleColor(const Value: TColor);
begin
  if (FSingleColor <> Value) then
  begin
    FSingleColor := Value;
    Changed;
  end;
...
procedure TSplitterAppearance.SetSingleColorHot(const Value: TColor);
begin
  if (FSingleColorHot <> Value) then
  begin
    FSingleColorHot := Value;
    Changed;
  end;
end;
...
constructor TCustomSplitterEx.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TCustomSplitterEx.DblClick;
var
  FControl: TControl;
begin
  inherited;
...
destructor TCustomSplitterEx.Destroy;
begin
  FAppearance.Free;
  FGlyph.Free;
  inherited;
...
procedure TCustomSplitterEx.CMMouseEnter(var Message: TMessage);
begin
  inherited;
...
procedure TCustomSplitterEx.AdjustNeighbourControl;
var
  p: TPoint;
  i, sz: Integer;
  Ctrl: TControl;
...
procedure TCustomSplitterEx.MouseDown(Button: TMouseButton;
...
procedure TCustomSplitterEx.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
function TCustomSplitterEx.FindControl: TControl;
var
  P: TPoint;
  I: Integer;
  R: TRect;
...
procedure TCustomSplitterEx.CMMouseLeave(var Message: TMessage);
begin
  inherited;
  if (csDesigning in ComponentState) then
    Exit;
...
procedure TCustomSplitterEx.Loaded;
begin
  inherited;
end;

procedure TCustomSplitterEx.OnAppearanceChanged(Sender: TObject);
begin
  Invalidate;
end;

procedure TCustomSplitterEx.OnGlyphChanged(Sender: TObject);
begin
  Invalidate;
end;
...
procedure TCustomSplitterEx.Paint;
var
  R: TRect;
  PS: integer;
  i: integer;
  ...
  begin
    if FStyle <> tsSolidColor then
       ...
       begin
         ...
         brClr:= clNone;
         R:= ClientRect;
         Canvas.Pen.Color:= Appearance.SingleColorHot;
         Canvas.Brush.Color:= Appearance.SingleColorHot;
  ...
  begin
    if FStyle <> tsSolidColor then
       ...
       begin
         ...
         brClr:= clNone;
         R:= ClientRect;
         Canvas.Pen.Color:= Appearance.SingleColor;
         Canvas.Brush.Color:= Appearance.SingleColor;
         Canvas.Brush.Style:= bsSolid;
         Canvas.Rectangle(R);
       end;
  end;
...
procedure TCustomSplitterEx.SetAppearance(
  const Value: TSplitterAppearance);
begin
  FAppearance.Assign(Value);
end;

procedure TCustomSplitterEx.SetComponentStyle(AStyle: TTMSStyle);
begin
  ...
  case AStyle of
    ...
    tsSolidColor:
      begin
        Appearance.Color:= Appearance.SingleColor;
        Appearance.ColorTo := Appearance.SingleColor;
        Appearance.ColorHot := Appearance.SingleColor;
        Appearance.ColorHotTo := Appearance.SingleColor;
        Appearance.DirectionType := dtAuto;
      end;
    tsCustom:
      begin
...
procedure TCustomSplitterEx.SetGlyph(const Value: TBitmap);
begin
  FGlyph.Assign(Value);
end;

procedure TCustomSplitterEx.SetGripStyle(const Value: TSplitterGripStyle);
begin
...
procedure TCustomSplitterEx.WMEraseBkGnd(var msg: TWMEraseBkGnd);
begin
  inherited;
end;

procedure TCustomSplitterEx.SetPopupMenu(Value: TPopupMenu);
begin
  FPopupMenu := Value;
  if Value <> nil then
  begin
...
function TCustomSplitterEx.GetPopupMenu: TPopupMenu;
begin
  Result := FPopupMenu;
end;
